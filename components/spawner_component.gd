...
#func spawn_projectile(target: Node2D, global_spawn_position: Vector2 = global_position, parent: Node = get_tree().current_scene) -> Projectile:
	#assert(scene is PackedScene, "Error: The scene export was never set on this spawner component.")
	#var spawn = spawn_node2d(global_spawn_position, parent)
